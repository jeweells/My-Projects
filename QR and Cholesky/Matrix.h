...
private:
	...
	class Proxy {
	public:
		Proxy(T* _arr, int columns, int x) : _arr(_arr), columns(columns), x(x) {}
		T& operator[](int y) {
			return _arr[y + columns * x];
		}
	...
	public:
		...
		void table() {
			...
			cout << endl;
			for (int i = m->columns; i < m->columns * m->rows; i++) {
				int border = toString(m->arr[i]).length();
				bool etc = false;
				if (border > column[i % m->columns])
					etc = true;

				for (int j = 0; j < column[i % m->columns]; j++) {
					if (j > border) {
						cout << " ";
					}
					else {
						if (j < (column[i % m->columns] - (etc ? 3 : 0)))
			...
			for (int i = 0; i < m->columns * m->rows; i++) {
				...
				for (int j = 0; j < column[i % m->columns]; j++) {
					if (j > border) {
						...
						if (j < (column[i % m->columns] - (etc ? 4 : 0)))
							cout << toString(m->arr[i])[j];
						else {
							cout << "...";
							break;
...
public:
	...
	Matrix(int rows = 0, int columns = 0) {
		if (rows > 0 && columns > 0) {
			...
			throw "Matrix initialization should have rows and columns arguments and must be greater than zero.";
		}
	...
	bool operator< (Matrix m) {
		...
		else if (this->columns *this->rows < m.columns * m.rows) {
			return true;
		}
		return false;
	}
	...
	Matrix& operator* (Matrix& m) {
		...
		throw "Error: The rows of first matrix should be equal to the columns of the second matrix in multiplication.\n";
	}
	template<class S> Matrix& operator/ (S x) {
		Matrix* result = new Matrix(this->rows, this->columns);
		for (int i = 0; i < this->rows; i++) {
	...
	Matrix& operator/ (Matrix& m) {
		if (this->columns == m.rows && m.columns == m.rows) {
			Matrix* result = new Matrix(this->rows, m.columns);
			(*result) = (*this) * (m ^ -1);
			return *result;
		}
		throw "Error: The rows of first matrix should be equal to the columns of the second matrix and second matrix should be square in derivation.\n";
	}
	Matrix& operator^ (int x) {
		if (this->rows == this->columns) {
			...
			throw "A matrix can raised to a certain power only if it has the same rows and columns length.\n";
		}
	}


	void set(int x, int y, T value) {
		arr[y + columns * x] = value;
	}
...
template<class T> double cofactor(Matrix<T> mat, int x, int y) {
	return pow(-1, x + y) * (mat.columns == 2 && x + y == 1 ? mat[x][y] : Minor(mat, x, y));
}
